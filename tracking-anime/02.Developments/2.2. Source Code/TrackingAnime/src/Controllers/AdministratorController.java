...
import Modeles.ManageAccount;
import Modeles.Schedule;
import Modeles.data;
...
public class AdministratorController implements Initializable {
    ...
    @FXML
    private TextField db_txtStudio;
    ...
    @FXML
    private TableColumn<Schedule, ToggleButton> sc_tbvcolNotification;
    ...
    public ObservableList<Database> getDashboardEntry() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                dbItems = new Database(image,rs.getString("title"),
                        rs.getInt("season"),rs.getInt("type"),rs.getInt("status"),
                        rs.getInt("nation"),rs.getDate("aried"),rs.getInt("episodes"),
                        rs.getInt("new_episode"),rs.getString("studio"),rs.getString("introduction"));
            ...
            return new TableCell<Database, Void>(){
                private final Button button = new Button();{
                    button.setOnAction(event -> {
                        int animeid = getTableRow().getIndex();
                        ...
                        try {
                            ...
                            rs = st.executeQuery();
                            while (rs.next()) {
                                Parent root = FXMLLoader.load(getClass().getResource("/Controllers/PopupAnimeAdminController.java"));
    ...
    public void addbtn(){
        if(db_txtPoster.getText().isEmpty() || db_txtTitle.getText().isEmpty() ||
                ...
                db_txtStudio.getText().isEmpty() || db_txtIntroduction.getText().isEmpty()){
            ...
            try {
                ...
                st.setString(8, db_txtStudio.getText());
    ...
    public void updatebtn(){
        if(db_txtPoster.getText().isEmpty() || db_txtTitle.getText().isEmpty() ||
            ...
            String updateData= "update Anime set poster= '" + db_txtPoster.getText() + "' ,title= '" + db_txtTitle.getText() + "' ,type= '"+
                    ...
                    "' ,episode= '" + db_txtEpisode.getText() + "' ,introduction= '" + db_txtIntroduction.getText() + "' ,Nation= '"
                    + db_cbNation.getSelectionModel().getSelectedItem().toString() + "' , new_episode= '" + db_txtNewepisode + "', studio = '" +
                    db_txtStudio.getText() + "'";
            try {
    ...
    public void deletebtn(){
        ...
        if(animeid != -1){
            ...
            Optional<ButtonType> option = alert.showAndWait();
            if (option.get().equals(ButtonType.OK)) {
                String deleteData = "delete from Anime where anime_id = '" + offset + "'";
                try {
    ...
    public void selecteditems(){
        ...
        try{
            ...
            while(rs.next()){
                ...
                db_txtStudio.setText(rs.getString("studio"));
    ...
    public void selectedschedules(){
        String getData = "SELECT Anime.anime_id,Anime.poster,Anime.status, Anime.title, Schedule.day, Schedule.time, Schedule.ischecked FROM Anime \n" +
    ...
    public void addschedulebtn(){
        ...
        sc_cbDay.getSelectionModel().getSelectedItem() == null){
            ...
            String getData = "SELECT * from Anime where anime_id= '" + sc_txtAnimeID.getText() + "'";
            try {
                st = cnn.prepareStatement(getData);
                rs = st.executeQuery();
                if(!rs.next()){
                    ...
                    alert.setTitle("Error Message");
                    alert.setHeaderText("");
                    alert.setContentText("No ID found in the Anime table");
                    alert.showAndWait();
                }else{
                            String insertScheduleData = "Insert into Schedule(anime_id,day,time) values(?,?,?)";
                            st = cnn.prepareStatement(insertScheduleData);
                            st.setInt(1, Integer.parseInt(sc_txtAnimeID.getText()));
                            st.setString(2, sc_cbDay.getSelectionModel().getSelectedItem().toString());
                            st.setString(3, sc_txtTime.getText());
                            st.executeUpdate();
                        // 20:00:00
                    alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Information Message");
    ...
    public ObservableList<Schedule> getScheduledata() {
        ...
        String query = "SELECT Anime.anime_id,Anime.poster,Anime.status, Anime.title,Schedule.schedule_id ,Schedule.day, Schedule.time, Schedule.ischecked FROM Anime " +
                "INNER JOIN Schedule ON Anime.anime_id = Schedule.anime_id";
        try{
            st = cnn.prepareStatement(query);
            rs = st.executeQuery();
            while(rs.next()){
    ...
    public void scheduleShowcase(){
        ...
        sc_tbvcolNotification.setCellFactory(column -> new TableCell<Schedule, ToggleButton>() {
            private final ToggleButton toggleButton = new ToggleButton();

            {
                toggleButton.setOnAction(event -> {
                    int currentIndex = getIndex();
                    Schedule schedule = getTableView().getItems().get(currentIndex);
                    int scheduleid = schedule.getSchedule_id();
                    try {
                        String updateChecked = "UPDATE Schedule SET ischecked = ? WHERE schedule_id = ?";
                        try (PreparedStatement st = cnn.prepareStatement(updateChecked)) {
                            st.setBoolean(1, toggleButton.isSelected());
                        ...
                        if (toggleButton.isSelected()) {
                            ...
                            toggleButton.getStyleClass().addAll("on-notify", "notify");
                        } else {
                            toggleButton.setText("OFF");
                            toggleButton.getStyleClass().clear();
                            toggleButton.getStyleClass().addAll("off-notify", "notify");
                        }
            ...
            protected void updateItem(ToggleButton item, boolean empty) {
                ...
                if (empty) {
                    ...
                    if (currentIndex >= 0 && currentIndex < getTableView().getItems().size()) {
                        ...
                        String query = "SELECT ischecked FROM Schedule WHERE schedule_id = ?";
    ...
    public ObservableList<ManageAccount> getManageAccountdata(){
        ...
        try {
            ...
            while(rs.next()){
                ...
                String formattedDate = dateFormat.format(rs.getDate("last_updated"));
